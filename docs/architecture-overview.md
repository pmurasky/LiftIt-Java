...
- `created_by BIGINT` — FK to `users.id`; nullable on the `users` table only (bootstrapping: the user does not exist yet at insert time)
- `updated_at TIMESTAMP WITH TIME ZONE NOT NULL`
- `updated_by BIGINT` — FK to `users.id`; nullable on the `users` table only (same bootstrapping reason)
...
| `created_at` | `TIMESTAMP WITH TIME ZONE NOT NULL` | | |
| `created_by` | `BIGINT` | NULLABLE, FK → `users.id` | Nullable due to bootstrapping: set to own `id` after insert |
| `updated_at` | `TIMESTAMP WITH TIME ZONE NOT NULL` | | |
| `updated_by` | `BIGINT` | NULLABLE, FK → `users.id` | Nullable due to bootstrapping |
...
#### `user_profiles`
Lifting-specific profile data. Created alongside the `users` row on first login or lazily during onboarding.
Planned fields: `user_id` (FK), `username`, `display_name`, `gender`, `birthdate`, `height_cm`, `body_weight_kg`, `units_preference` (metric/imperial).
...
Depends on `user_profiles` being implemented first.
